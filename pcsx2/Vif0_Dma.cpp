...
__fi void vif0SetupTransfer()
{
	...
	ptag = dmaGetAddr(vif0ch.tadr, false);

	if (!(vif0ch.transfer("vif0 Tag", ptag))) return;

	vif0ch.madr = ptag[1]._u32;
	g_vif0Cycles += 1;
	...
	if (vif0ch.chcr.TTE)
	{
		...
		if (vif0.irqoffset.enabled)
		{
			ret = VIF0transfer((u32*)&masked_tag + vif0.irqoffset.value, 4 - vif0.irqoffset.value, true);
		}
		else
		{
			vif0.irqoffset.value = 2;
			vif0.irqoffset.enabled = true;
			ret = VIF0transfer((u32*)&masked_tag + 2, 2, true);
		}
